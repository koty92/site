...
class Api_controller extends CI_Controller {
	...
	function __construct() {
		parent::__construct();
		$this->load->model(array('api/v5/api_model'));
	...
	public function fetch_merchant_home() {
		...
		if(
			...
			// Get promo pages
			$pages = $this->api_model->get_promo_pages($companyPid);
	...
	public function fetch_promo_page() {
		$json = file_get_contents('php://input');
		$obj = json_decode($json, true);
		...
		if($this->authorizeApp($appToken)) {
			...
			// Get promotions
			$promo = $this->api_model->get_promotions($categoryPid, $page);
	...
	public function campaign_unlike() {
		$json = file_get_contents('php://input');
		$obj = json_decode($json, true);
	...
	public function fetch_user_liked_ads() {
		$json = file_get_contents('php://input');
		$obj = json_decode($json, true);
		...
		if($this->authorizeApp($appToken) && $this->authorizeUser($userToken, $userPid)) {
			$liked_ads = $this->api_model->get_user_liked_ads($userPid);

			$callback['success'] = '1';
			$callback['liked_ads'] = $liked_ads;
		} else {
			$callback['success'] = '2';
			$callback['msg'] = 'Unauthorized App';
		}
	...
	public function fetch_user_favorites() {
		$json = file_get_contents('php://input');
		$obj = json_decode($json, true);
		...
		if($this->authorizeApp($appToken) && $this->authorizeUser($userToken, $userPid)) {
			$favorites = $this->api_model->get_user_favorites($userPid);
	...
	public function signup_profile() {
		...
		if($this->authorizeApp($appToken) && $this->authorizeUser($userToken, $userPid)) {
			...
			$first_name = explode(' ', $obj['user_name']);
			$referral = strtolower($first_name[0] . $userPid);
	...
	public function fetch_promo_detail() {
		...
		if($this->authorizeApp($appToken)) {
			...
			// Get ads like
			$like = $this->api_model->get_campaign_like($campaignPid);

			// Get ads status
			$like_status = $this->api_model->get_like_status($campaignPid, $userPid);
			...
			if($result) {
				...
				$callback['view'] = $view;
				$callback['like'] = $like;
				$callback['like_status'] = $like_status;
			} else {
				$callback['success'] = '0';
			}
		} else {
	...
	public function redeem_rewards() {
		...
		if($this->authorizeApp($appToken) && $this->authorizeUser($userToken, $userPid)) {
			...
			$redeemAmount = $obj['redeemAmount'];
			$redeemName = $obj['redeemName'];
			$redeemNumber = $obj['redeemNumber'];
			...
			if($point >= $redeemAmount) {
				$data = array(
					'redeem_datetime' => $datetime,
					'user_pid' => $userPid,
					'redeem_amount' => $redeemAmount,
					'redeem_number' => $redeemNumber,
					'redeem_name' => $redeemName
				);

				if($this->db->insert('redeem', $data)) {
					// Deduct points
					$this->db->set('user_point', 'user_point-' . $redeemAmount, FALSE);
					$this->db->where('user_pid', $userPid);
					$this->db->update('data_user');

					$newPoint = $point - $redeemAmount;
					
					$callback['success'] = '1';
					$callback['msg'] = 'Reward has been redeemed';
					$callback['point'] = $newPoint;
				}
			} else {
				$callback['success'] = '0';
